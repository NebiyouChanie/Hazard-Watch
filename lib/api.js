...
export const fetchHazardData = {
  ...
  // Add to existing fetchHazardData object in lib/api.js

// Rainfall anomaly endpoints
getRainfallAnomaly: async (date, period = 'daily') => {
  let endpoint;
  switch (period) {
    case 'daily':
      console.log("🚀 ~ getRainfallAnomaly: ~ period:", period)
      endpoint = `${API_BASE_URL}/rainfall/anomaly/daily/${date}`;
      break;
    case 'monthly':
      endpoint = `${API_BASE_URL}/rainfall/anomaly/monthly/${date}`;
      break;
    case 'annual':
      endpoint = `${API_BASE_URL}/rainfall/anomaly/annual/${date}`;
      break;
    case 'seasonal':
      endpoint = `${API_BASE_URL}/rainfall/anomaly/seasonal/${date}`;
      break;
    default:
      throw new Error(`Invalid period: ${period}`);
  }
...
getAnomalyTimeSeries: async (aggregation = 'daily') => {
  const res = await fetch(`${API_BASE_URL}/timeseries/anomaly?aggregation=${aggregation}`);
  if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
  return res.json();
},

getDailyAnomalyTimeSeriesByDay: async (date) => {
  const res = await fetch(`${API_BASE_URL}/timeseries/anomaly/daily_by_day?date=${date}`);
  if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
  return res.json();
},

getMonthlyAnomalyTimeSeries: async (month) => {
  const url = month 
    ? `${API_BASE_URL}/timeseries/anomaly/monthly?month=${month}`
    : `${API_BASE_URL}/timeseries/anomaly/monthly`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
  return res.json();
},

getAnnualAnomalyTimeSeries: async () => {
  const res = await fetch(`${API_BASE_URL}/timeseries/anomaly/annual`);
  if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
  return res.json();
},

getSeasonalAnomalyTimeSeries: async (season) => {
  const url = season 
    ? `${API_BASE_URL}/timeseries/anomaly/seasonal?season=${season}`
    : `${API_BASE_URL}/timeseries/anomaly/seasonal`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
  return res.json();
},
